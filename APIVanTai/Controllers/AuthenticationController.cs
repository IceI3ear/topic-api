    ...
    public class AuthenticationController : ControllerBase
    {
        ...
        public async Task<IActionResult> Login([FromBody] Login model)
        {
            ...
            if(user!=null && await userManager.CheckPasswordAsync(user, model.Password))
            {
                ...
                var classInfo = db.Classes.Include(x=>x.Speciality).ToList();
                var userInfo = db.AspNetUsers.Where(x => x.Id == user.Id).Include("Class").FirstOrDefault();
                var topic = db.Topics.Where(x => x.StudentID == user.Id).FirstOrDefault();
                return Ok(new
                {
                    ID = userInfo.Id,
                    ...
                    MajorName = classInfo.Where(x=>x.ID == userInfo.ClassID).Select(x=>x.Speciality.Name),
                    MajorID = classInfo.Where(x => x.ID == userInfo.ClassID).Select(x => x.SpecialityID),
                    Type = userInfo.Type,
                    TeacherID = userInfo.TeacherID,
                    IsCheckTopic = topic != null? true: false,
                    TopicÌD = topic != null ? topic.ID : 0,
